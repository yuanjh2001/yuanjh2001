...
public class UpdateService extends BaseService{
    ...
    private Map<String, String> getParams(String timespan){
    ...
    private void httpsCheckUpdate() {
        Request<String> httpRequest = NoHttp.createStringRequest(apiUrl, RequestMethod.GET);
        httpRequest.setHostnameVerifier(SSLContextUtil.HOSTNAME_VERIFIER);
        String timespan = (System.currentTimeMillis() / 1000) +"";
        Map<String, String> map = getParams(timespan);
        Map<String, String> singMap = getParams(timespan);
        singMap.put("appsecret",appsecret);
        ...
        CallServer.getRequestInstance().add(mContext, 0, httpRequest, new HttpListener<String>() {
            ...
            public void onSucceed(int what, Response<String> response) {
                ...
                UpdateApkResponseVo vo = null;
                try {
                    vo = new Gson().fromJson(response.get(), UpdateApkResponseVo.class);
                }catch (Exception e){
                    ToastUtils.createToast(mContext,"网络异常或服务器数据错误！");
                }
                if (vo != null && vo.getResult() != null) {
                    ...
                    if ("0".equals(vo.getResult().getRet())) {
                        try {
                            ...
                            LogUtils.i("systemCode " + systemCode + " version " + vo.getVersion() + " appversion " + updateVersion + " appdownloadurl " + vo.getAppdownloadurl() + " isCompulsoryUpdate " +vo.isCompulsoryUpdate());
                            ...
                            if (updateVersion > systemCode && (!isIgnoring() || isCompulsoryUpdate)) {
                                ...
                                if(showTips){
                                    if (updateVersion > systemCode){
                                        showUpdateApkDialog();
                                    }else {
                                        ToastUtils.createToast(mContext,"当前已是最新版本！");
                                    }
                                }
                            }
                        } catch (PackageManager.NameNotFoundException e) {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if(intent != null && intent.getExtras() != null){
            ...
            showTips = intent.getExtras().getBoolean("showTips");
            httpsCheckUpdate();
        }
